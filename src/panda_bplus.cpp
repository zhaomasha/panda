...
//读取索引文件，还原一个内存中的索引结构
/*void Btree::recover(string name){
	filename=name;
...
//计算块对应的文件偏移
f_type Btree::get_offset(b_type num){
	return sizeof(BtreeHeader)+num*head.block_size;
}
//对文件新增的部分建立索引，文件新增的部分还没有取到内存中，否则会有一致性的问题
void Btree::update_index(){
	//计算新增部分的块的数目
	io.seekg(0,fstream::end);
	f_type file_size=io.tellg();
	b_type num=head.block_num;
	b_type blocks=((long)file_size-sizeof(BtreeHeader)-head.block_size*num)/head.block_size;
...
b_type Btree::require(uint32_t type){
	if(head.free_num==0){
...
b_type Btree::requireRaw(uint32_t type){
	...
	if(type==1){
		//叶子节点，初始化节点的容量
		((Bleaf<char,char>*)block)->init(type,head.block_size);
	}

	if(type==2){
		//非叶子节点，初始化节点的容量
		((Bnoleaf<char>*)block)->init(type,head.block_size);
	}	
...
void Subgraph::add_edge(v_type id,Edge &e,char is_hash){
	...
	BlockHeader<Edge>* block=(BlockHeader<Edge>*)get_block(b_num,0,is_hash);
...
b_type Subgraph::index_edge(Vertex* v,v_type id,b_type num,char is_hash){
	...
	if(v->index==INVALID_BLOCK){
		...
		BlockHeader<Vertex> *bv=(BlockHeader<Vertex> *)get_block(num,0,is_hash);
		bv->clean=1;
                //获取新分配的索引块
	...
	while(true){
		if((b->min==INVALID_VERTEX)||(id<b->min)){
			...
			b=(BlockHeader<Index> *)get_block(b->next,0,is_hash);
			(b->fix)++;//盯住新的块
		}
	}
...
*/
void* Btree::get_block(b_type number){
	...
	if(node_it!=cache.end()){
		...
		if(!(cache.size()<atoi(getenv("CACHESZ")))){
			...
			while(node!=NULL){
				...
				if(((Bnode<char>*)(node->block))->fix==0) break;
				else node=node->pre;
			}
			if(node==NULL) {
		...
		if(((Bnode<char>*)block)->type==1){
			((Bleaf<char,char>*)block)->init_mem();
		}
		if(((Bnode<char>*)block)->type==2){
			((Bnoleaf<char>*)block)->init_mem();
		}
		//把块加入缓存中
		cache[number]=node;
		//更新链表
